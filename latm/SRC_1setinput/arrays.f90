...
!!!FN
  ...
  COMPLEX(DPC), ALLOCATABLE :: calVlda(:,:,:)
...
!!!
    ...
    IF (layeredCalculation) THEN
       ...
       ELSE
          ...
          STOP
       END IF
       if(debug)WRITE(*,*) "Allocating calVlda(3",nMax,",",nMax,")"
       ALLOCATE (calVlda(3,nMax,nMax), STAT=istat)
       IF (istat.EQ.0) THEN
          istat=0
          if(debug) WRITE(*,*) 'Allocated array calVlda'
       ELSE
          WRITE(6,*) 'Could not allocate calVlda'
          WRITE(6,*) 'Stopping'
          STOP
       END IF
    END IF
...
!!!#BMSu
    ...
    IF (layeredCalculation) THEN
       ...
       IF (istat.NE.0) THEN
          ...
          STOP
       END IF
       DEALLOCATE (calVlda, STAT=istat)
       IF (istat.NE.0) THEN
          WRITE(6,*) 'Could not deallocate calVlda'
          WRITE(6,*) 'Stopping'
          STOP
       END IF
    end IF
