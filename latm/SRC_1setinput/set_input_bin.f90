...
!!! BMS/FN
  ...
  ! debug
  if(vnlkss)then
     write(*,*)'set_input_bin.f90: vnl will be included'
  else
     write(*,*)'set_input_bin.f90: vnl not included'
  end if
  !stop 'set_input_bin.f90:stop'
  !#BMSVer3.0u
!  write(*,*)"%%%%%%%%%%%%%%%%%%%%%%"
...
!  IF (nSpinor == 2) THEN
  ...
  if ( spinCalculation) then
     ...
     OPEN(41, FILE=smn_data_filename, STATUS='OLD', IOSTAT=io_status, FORM="unformatted")
     IF (io_status /= 0) THEN
        WRITE(6,*) "Error occured trying to open:", smn_data_filename
        WRITE(6,*) "Error status returned is:", io_status
        WRITE(6,*) "Stopping"
        STOP "Stopping: error with momentum matrix element file"
     else
        WRITE(6,*) "no problem opening:",smn_data_filename
     END IF
  !ELSE
...
!  END IF
  ...
  IF ( layeredCalculation ) THEN
     ...
     OPEN(16, FILE=cal_data_filename, STATUS='OLD', IOSTAT=io_status, FORM="unformatted")
     IF (io_status /= 0) THEN
        WRITE(6,*) "Error occured trying to open:", cal_data_filename
        WRITE(6,*) "Error status returned is:", io_status
        WRITE(6,*) "Stopping"
        STOP "Stopping: error with momentum matrix element file"
     else
        WRITE(6,*) "no problem opening:", cal_data_filename
     END IF
  !else
  ...
  IF ( layeredCalculation ) THEN
     ...
     OPEN(53, FILE=cfmn_data_filename,STATUS='OLD',IOSTAT=io_status, FORM="unformatted")
     IF (io_status /= 0) THEN
        WRITE(6,*) "Error occured trying to open:", cfmn_data_filename
        WRITE(6,*) "Error status returned is:", io_status
        WRITE(6,*) "Stopping"
        STOP "Stopping: error with momentum matrix element file"
     else
        WRITE(6,*) "no problem opening:",cfmn_data_filename
     END IF
  !else
  ...
  IF ( layeredInjectionCurrent ) THEN 
     ...
     OPEN(17, FILE=cur_data_filename,STATUS='OLD',IOSTAT=io_status, FORM="unformatted")
     IF (io_status /= 0) THEN
        ...
        WRITE(6,*) "no problem opening:",cur_data_filename
     END IF
  !else
  ...
  IF ( ndotCalculation ) THEN
     ...
     OPEN(69, FILE=rhomm_data_filename,STATUS='OLD',IOSTAT=io_status, FORM="unformatted")
     IF (io_status /= 0) THEN
        WRITE(6,*) "Error occured trying to open:", rhomm_data_filename
        WRITE(6,*) "Error status returned is:", io_status
        WRITE(6,*) "Stopping"
        STOP "Stopping: error with momentum matrix element file"
     else
        WRITE(6,*) "no problem opening:",rhomm_data_filename
     END IF
  !else
  ...
  DO ik = 1, kMax
     ...
     DO iv = 1, nMax
        ...
        DO ic = iv, nMax
           ...
           !write(*,*)'set_input_bin.f90: ',io_status,' unit 11'
...
!           write(70,*)'in set_input_bin.f90:',scissor
           DO ii=1,3
              ! v^\lda=p/m + v^\nl
     ...
     if (1.eq.1)then
        IF ( layeredInjectionCurrent ) THEN
           do iv=1,nMax
              do ii=1,3
                 if(vnlkss)then
                    if((ik.eq.1).and.(iv.eq.1).and.(ii.eq.1))write(*,*)"set_input_bin.f90: layeredInjectionCurrent with vnl"
                    if ( scissor .gt. 0.d0 ) then
                    ...
                    if((ik.eq.1).and.(iv.eq.1).and.(ii.eq.1))write(*,*)"set_input_bin.f90: layeredInjectionCurrent without vnl"
