...
!!!========== integrand ============
!!!========== integrand ============
 ALLOCATE (ctmp3(3,50,5000), STAT=istat)
   if (istat.NE.0) then
       write(6,*) 'Could not allocate momMatElem'
       write(6,*) 'Stopping'
       stop
...
!   write(*,*)"ik=",ik," calculating Integrand "
     do  iv = 1, nVal
       do ic = nVal+1, nMax
          iw =int((energy(ik,ic)-energy(ik,iv))/delta) !! here we dont know the size of the grid !!!!
          ctmp3(1,ik,iw) = ctmp3(1,ik,iw) + posMatElem(1,ic,iv)*posMatElem(1,iv,ic) !! just one comp. xx
...
do iw=  1, iwMax
   ...
   write(31,*)(iw*delta), real(ctmp),aimag(ctmp)
end do
