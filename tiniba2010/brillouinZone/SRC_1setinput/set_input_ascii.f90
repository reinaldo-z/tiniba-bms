...
!!!BMS
  ...
  DO ik = 1, kMax
     !IF ((ik<11).OR.(MOD(ik,50).EQ.0).OR.(ik.EQ.kMax)) THEN
        WRITE(6,*)'AT BEGINING set_input ik= ',ik
     !END IF
...
!     DO iv = 1, nMax
!        DO ic = iv, nMax
!                 WRITE(171,103) REAL(momMatElem(1,iv,ic)),IMAG(momMatElem(1,iv,ic)) &
...
DO ik = 1, kMax
  DO iv = 1, nVal
       DO ic = nVal+1, nMax
       WRITE(156,103) REAL(momMatElem(1,iv,ic)),IMAG(momMatElem(1,ic,iv)) &
