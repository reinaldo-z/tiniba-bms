...
!!! SUBROUTINE calculateintegrands
  SUBROUTINE calculateintegrands(comova)
!!!###########################
    IMPLICIT NONE
    
    CHARACTER(LEN=3) :: ADV_opt
    REAL(DP) :: omegamp, omeganp, omegapn
    integer, intent(in) :: comova
    !--------------------------------------------------------------------------
...
!!!!!!!!!!!!
    DO iv = 1, nVal
       DO ic = nVal+1, nMax
                   write(380,*)punto,momMatElem(1,ic,iv), momMatElem(1,iv,ic),&
...
!!              write(180,*)momMatElem(1,ic,iv), momMatElem(1,iv,ic),&
!!                          momMatElem(2,ic,iv), momMatElem(2,iv,ic),&
!!                          momMatElem(3,ic,iv), momMatElem(3,iv,ic)
!!
!
...
!     write(49,"(3I,2E15.7)")punto,iv,ic,real(momMatElem(ix,ic,iv)*momMatElem(iy,iv,ic)), aimag(momMatElem(ix,ic,iv)*momMatElem(iy,iv,ic))
!     write(49,"(3I,2E15.7)")punto,iv,ic,real(momMatElem(ix,ic,iv)*momMatElem(iy,iv,ic)), aimag(momMatElem(ix,ic,iv)*momMatElem(iy,iv,ic))

!               !write(180,*)momMatElem(ix,ic,iv)*momMatElem(iy,iv,ic)
!
             END DO
       END DO
          
!         ! write(59,"(3I)")punto,iv,ic!,momMatElem(1,ic,iv)*momMatElem(1,iv,ic)
!          write(59,"(3I,1E15.7)")punto,iv,ic,real(momMatElem(1,ic,iv)*momMatElem(1,iv,ic))     
          IF (ic==nMax) THEN
             WRITE(UNIT=spectrum_info(i_spectra)%integrand_filename_unit, &
...
!    REAL(DP) :: T2(3,3),T1(3)
    ...
    T3(1:3,1:3,1:3) = reshape(spectrum_info(i_spectra)%transformation_elements(1:27), (/3,3,3/))    
!    T2(1:3,1:3) = reshape( spectrum_info(i_spectra)%transformation_elements(1:9), (/3,3/) )
!    T1(1:3) = reshape( spectrum_info(i_spectra)%transformation_elements(1:3), (/3/) )
!    write(*,*)T2(1:3,1:3)
!!!!!!!!!!!!!!!!!
     write(*,*)"kpoint",kpoint
    ik=ik+1
...
!
                   if (iv.ne.ic)then 
                     ! tmp = tmp +  T3(ix,iy,iz)*(curMatElem(ix,ic)-curMatElem(ix,iv))* & 
                     !      (PosMatElem(iy,ic,iv)*PosMatElem(iz,iv,ic))
                       tmp = tmp +  1*(curMatElem(ix,ic)-curMatElem(ix,iv))* & 
                           (PosMatElem(iy,ic,iv)*PosMatElem(iz,iv,ic))
!                       tmp = tmp +  PosMatElem(iy,ic,iv)*PosMatElem(iz,iv,ic)
                   end if
!!!!!!!!!!!! Modificated according to BMS Lunes 17 Agosto 2009 ec. 16
!!!!!!!!!!!! Modificated according to BMS Lunes 17 Agosto 2009 
                END DO
          ...
          IF (ic==nMax) THEN
             WRITE(UNIT=spectrum_info(i_spectra)%integrand_filename_unit, &
                  FMT="(E15.7)",ADVANCE="YES")aimag(tmp)!aimag(tmp)
             !WRITE(58,"(1i5,E15.7)",ADVANCE="YES")kpoint, aimag(tmp)!aimag(tmp)
                  !WRITE(57,"(3i4,2e15.7)")kpoint,iv,ic,aimag(tmp),real(tmp)
              ELSE
             WRITE(UNIT=spectrum_info(i_spectra)%integrand_filename_unit, &
                  FMT="(E15.7)",ADVANCE="NO")aimag(tmp)!aimag(tmp)
            ! WRITE(58,"(E15.7)",ADVANCE="NO")aimag(tmp)
                 !WRITE(57,"(3i4,2e15.7)")kpoint,iv,ic,aimag(tmp),real(tmp)
          END IF
