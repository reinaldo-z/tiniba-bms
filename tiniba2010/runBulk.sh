...
## copyAfterRpmns.sh
## menu.sh
## LAST MODIFICATION :  Febrero 18 2010 a 11:11 by Cabellos
## LAST MODIFICATION :  Febrero 22 2010 a 17:33 by Cabellos
## LAST MODIFICATION :    March 23 2010 a 09:54 by Cabellos
## LAST MODIFICATION :    April 06 2010 a 17:08 by Cabellos
## LAST MODIFICATION :    June  07 2010 a 13:49 by Cabellos
...
##<><><><><><><><><><><><><><><>
     ...
     printf "\n"
     Line
     printf "\tWhen  : Today  `date`\n"
     printf "\tWho   : $USER \n"
     printf "\tWhere : $PWD\n"
     printf "\t${GREEN}DOWN DOWN DOWN DOWN DOWN DOWN DOWN DOWN${NC}\n" 
     echo -e "        How   : `dirname $0`/${GREEN}`basename $0`${NC} $@"
     printf "\tTaking all scripts from: $WHERE\n"
     printf "\tThere is not Garranty .... JL Cabellos\n"
     printf "\tReport bugs to <sollebac@hotmail.com>.\n"
...
#        Duerme 1
        Line
        printf "${MAG}Usefull commands us at your own will${NC}\n"
        ...
        printf "$WHERE/${GREEN}alive.sh${NC} \n"   
      ...
      Line
       ...
       printf "\t#$WHERE/`basename $0` "
       printf "${GREEN}run $nk  2 ${NC}${BLUE}3 1 ${NC} ## option 3 1\n"
       done
       StopMe
     fi 
...
####====================================
# capelly
###$WHERE/driver_copySCF.sh
$WHERE/copySCF2nodesWithCheck.sh
$WHERE/driver_abinisCreate.sh
$WHERE/driver_copyCase.sh
  TIMESTARTABINIS=`date`
  Line
  printf "\tRunning: abinis(serial) \n"
$WHERE/driver_abinisRun.sh
  sleep 2
##################################################
####################  WAIT  ABINIS  ##############
################################################## 
Line
  ...
  for ((hh=0;hh<=($NOMACHINESpmn-1); hh++));do
    ...
    DONDECORRE=${WHEREWORKREMOTE[$hh]}
   DONDEWORKLO=${WHEREWORKLOCAL[$hh]}
...
##################################################  
 ...
 MKrpmnEND="rsh $HOST 'cd $DONDEWORKLO;touch rpmnEND'"
  ...
  if [ "$ESPINsetUp" == "1" ];then
     rsh -n $MAQUINA502 "cd $DONDECORRE;$RPMNSEXEC -ene -mme $WF2DATASET > logfile;$MKrpmnEND;touch rpmnEND"&
  OPT1=0
  OPT2=2
  else 
     rsh -n $MAQUINA502 "cd $DONDECORRE;$RPMNSEXEC -ene -mme -sme $WF2DATASET > logfile;$MKrpmnEND;touch rpmnEND"& 
     fi 
...
ESPINsetUp=`grep nspinor setUpAbinit_$CASO.in  |  awk '{print $2}'`     
   ...
   if [ -e energy.d ];then
       mv energy.d eigen_$NKPT$LASTNAME
...
### keep memory about you run
  ...
  printf "\t--------------------------------------------------\n"
      ...
      printf "\t take:   $TMIN1 Hrs. \n"
      printf "\t--------------------------------------------------\n"
printf "\t${GREEN}UP UP UP UP UP UP UP UP UP UP UP UP UP${NC}\n" 
     echo -e "        How   : `dirname $0`/${GREEN}`basename $0`${NC} $@"
printf "\t I have find the end...check it please ....\n"
