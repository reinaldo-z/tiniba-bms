   ...
   SETUPABINIT=setUpAbinit_"$CASO".in
    ...
    PADREAMARO=`basename $WHERE`
      ...
      laheylib="env LD_LIBRARY_PATH=/usr/local/lf9562/lib"
          ...
          for ((hh=0;hh<=($NOMACHINESres-1); hh++));do        
           ...
           if [ "$NOBASICOPTIONS" -ne "$BO" ];then
              ...
              printf "[${GREEN}total${NC}] "
              printf "[${GREEN}CASO${NC}] [${GREEN}bandGap${NC}] [B] "
              printf "[${GREEN}BANDAS VALENCIA${NC}] [${GREEN}BANDAS CONDUCCION${NC}] "
              printf "[${GREEN}NUMBER RESPONSE${NC}] [TENSOR]\n"
     ...
     tolSHGt=${BASICOPTIONS[3]}
       TOTAL=`echo ${BASICOPTIONS[4]} | tr A-Z a-z`
       PFILE=${BASICOPTIONS[5]}
     BANDGAP=${BASICOPTIONS[6]}
         ISB=${BASICOPTIONS[7]}
        NBVA=${BASICOPTIONS[8]}
        NBCO=${BASICOPTIONS[9]}
     WHATANS=${BASICOPTIONS[10]}
     PMNFILE=pmn_"$PFILE"
            ...
            BUSCAPIES=0
              for ((jj=0;jj<=($NOWHOKK-1); jj++));do       
                ...
                if [ $BUSCAPIES -ne 0 ];then 
                     ...
                     CAB=${NAMERES[$WHATANS]}".kk"$AP"_"$NBCO"_"$PFILE
                     ...
                     CAB=${NAMERES[$WHATANS]}$AP"_"$NBCO"_"$PFILE
