...
#    NOT2XYZ=`echo ${#T2XYZ[@]}` 
 ...
 FILES="*kk*"
     ...
     for ((jj=1;jj<=($NOGFILES); jj++));do
        ...
        touch tempsal
        cp "$stringZ"  tempin 
        $RUNsmear 1  tempin tempsal $FWHM 
       #  if [ -e fromSmear ];then
       ...
       #   FASME=`echo ${nueva1s/./-}`
       #  fi
       #   SALIDAA=`echo ${OUTPUT/sm/sm_"$FASME"}` 
       #   mv tempsal $SALIDAA 
         mv tempsal $OUTPUT  
         printf "\t$jj.- $stringZ [input]\n"
         printf "\t$jj.- ${GREEN}$OUTPUT${NC} [output]\n"
         #printf "${GREEN}=================================${NC}\n" 
         rm -f tempsal
         rm -f tempin 
     done 
...
#        printf "t[${GREEN}$jj${NC}] $stringZ  ${GREEN}$SALIDAA${NC}\n"
#        mv $OUTPUT $SALIDAA
#
#     done
