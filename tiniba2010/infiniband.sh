...
if [ ! -e $IN ];then
 ...
 printf "\tYou original $IN has a $NOMUERTOS node dead  that has been eliminated\n"
...
if [ "$ANFIBIO" == "quad01" ];then 
 ...
 printf "\t ${RED}Hold on !${NC}\n"
 printf "\t Trying to continue with infiniband ...\n"
...
######
   ...
   touch $IN
   if [ -z "$QUEPEX" ];then 
   printf "\t Infiniband working in alives nodes is ok. and your final list is:\n"
