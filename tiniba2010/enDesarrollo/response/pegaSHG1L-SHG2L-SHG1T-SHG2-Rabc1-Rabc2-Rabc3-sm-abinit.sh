  ...
  # shg1_Length_ER+RA shg1_Length
   ...
   SHG=$1
    ...
    if [ "$SHG" == "both" ];then 
        printf "\t look for file: shg1_Tran and shg2_Tran  $KK \n"
        printf "\t look for file: shg1_Length and shg2_Length $KK \n"
        printf "\t look for file: Rabc_1 Rabc_2 Rabc_3 $KK \n"
    fi 
...
################
   ...
   # printf "                              ${MAG}$APELLIDO1${NC}\n"
    ...
    ### rigth now look for the same apellido in shg2L
                       ...
                       ### rigth now look for the same apellido in shg2T 
                        for ((iz=1;iz<=($NOSHG2T); iz++));do
                         ...
                         APELLIDO4=`echo ${stringSHG2T:$WHAT:$QUE}`
                          ...
                          #    IJKL=1
                          #   for ((uu=1;uu<=($NORABC); uu++));do
                          #     CADE=${RABC[$uu]}
                          #     LO=`expr length $CADE`  
                          #     #echo $LO
                          #     ARABC=`echo ${CADE:6:$LO}`
                          #     #echo $CADE
                          #     if [ $ARABC == $APELLIDO4 ];then
                          #     RABCGOOD[$IJKL]=$CADE
                          #     let "IJKL=IJKL+1"
                          #      echo $CADE
                          #     fi
       ...
       #### cambia aqui el nombre 
       #### cambia aqui el nombre 
FUERA="shg1L_shg2L_shg1T_shg2T_Rabc1_Rabc2_Rabc3$APELLIDO1"-abinit""
                               ...
                               #echo "FILE5=\"${RABC[1]}\"" >> paste_$BBRR.sh
                               #echo "FILE6=\"${RABC[2]}\"" >> paste_$BBRR.sh
                               #echo "FILE7=\"${RABC[3]}\"" >> paste_$BBRR.sh
                             ...
                             #  RABCGOOD[$IJKL]=$CADE
                                echo  $CADE
                                #echo "FILE$IJLK"
                                echo "FILE$IJLK=\"$CADE\"" >> paste_$BBRR.sh
                               #echo "FILE$IJLK=\"$CADE\"" >> paste_$BBRR.sh
                               #echo "FILE$IJLK=\"$CADE\"" >> paste_$BBRR.sh
                                #echo "FILE$IJLK=\"$CADE\"" >> paste_$BBRR.sh
                                let "IJLK=IJLK+1"
...
##  
 echo "awk '{print \$2,\$3}' \$FILE5 > tmp5" >>paste_$BBRR.sh  
 echo "awk '{print \$2,\$3}' \$FILE6 > tmp6" >>paste_$BBRR.sh
 echo "awk '{print \$2,\$3}' \$FILE7 > tmp7" >>paste_$BBRR.sh    
 echo "paste tmp1 tmp2 tmp3 tmp4 tmp5 tmp6 tmp7 > \$OUTPUT" >>paste_$BBRR.sh   
