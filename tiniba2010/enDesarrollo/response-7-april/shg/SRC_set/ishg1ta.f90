                ...
                DO dc=1,3
                   do l=1,nMaxCC
                      if((l.ne.v).and.(l.ne.c))then
                        ...
                        !!tmp=tmp+(T3(da,db,dc)/omegacv**3)*(aimag(momMatElem(da,l,c)*psym1)/(omegacl-2.*omegacv) -aimag(momMatElem(da,v,l)*psym2)/(omegalv-2.*omegacv)) 
...
!!! subroutine 
  COMPLEX(DPC) FUNCTION efe(c,v,i,j)
    ...
    ci=cmplx(0.d0,1.d0)
    !!!deltas=1.046d0 !!! tijeras a mano 
     !!!deltas=0.0d0 
     !!! deltas=6.999567592202993E-012 !!! 
    efe=cmplx(0.d0,0.d0)
    do vp=1,nVal
       if (vp.ne.v) then
   ...
   !!efe=-deltas*(ci*efe+derMatElem(j,i,c,v))  !!! this is tijeras by hand
