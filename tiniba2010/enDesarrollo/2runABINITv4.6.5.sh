...
if [ "$ARCH" == "quadp" ];then 
  ARQ="quad" 
   if [ ! -e machinesQUAD ];then
       printf "\tThere is not FILE: machinesQUAD, Im going to do one now ... \n"
        NOMAQ501=`echo ${#MAQ501[@]}`
         jj=0
          rm -f machinesQUAD ## paranoia jl
           touch machinesQUAD
            for ((hh=1;hh<=($NOMAQ501); hh++));do       
              MAQ502=${MAQ501[$hh]}
               if [[ "$MAQ502" == "$ARQ"* ]]; then
                echo "$MAQ502" >> machinesQUAD
               fi 
            done 
   fi 
    rm -f tmpQ 
        ...
        mv tmpQ machinesQUAD
         rm -f tmpQ
          NMA=`wc machinesQUAD | awk '{print$1}'` 
           printf "\t========================================\n"
           printf "\tIm going to run in ${GREEN}$NMA${NC} machines that are found in file machinesQUAD\n"
fi ### 
################################
################################
################################
