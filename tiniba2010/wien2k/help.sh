#!/bin/bash
## LAST MODIFICATION : Martes 02 Marzo 2010 by JL Cabellos
      RED='\e[0;31m'
     BLUE='\e[0;34m'
      BLU='\e[1;34m'
     CYAN='\e[0;36m'
...
printf "\t$WHERE/wien2k.sh klist \n"
printf "\t$WHERE/wien2k.sh run \n"
...
printf "\t (example of GaAs bulk )\n"
printf "\t 1.- gaas.struct\n"
printf "\t 2.-.machines    \n"
printf "\t 3.-.machines_pmn  \n"
printf "\t 4.-.machines_scf  \n"
printf "\t 5.-.machines_res  \n"
printf "\t 6.-.machines_latm \n"
printf "\t 7.-setUpAbinit_gaas.in\n"
printf "\t 8.-gaas.xyz\n"
printf "\t You can copy from: \n"
if [ -e "$WHERE/files/gaas.struct" ];then
printf "\tcp $WHERE/files/gaas.struct .\n"
fi  
if [ -e "$WHERE/files/.machines" ];then 
printf "\tcp $WHERE/files/.machines .\n"
fi 
if [ -e "$WHERE/files/setUpAbinit_gaas.in" ];then 
printf "\tcp $WHERE/files/setUpAbinit_gaas.in .\n"
fi 
