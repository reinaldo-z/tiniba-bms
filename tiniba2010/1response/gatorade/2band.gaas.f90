...
implicit none 
  ...
  CHARACTER(LEN=255) ::  coma,sigma !caso 1=s1.5, 2=s0, 3=s-1.5
  INTEGER :: istat, ArgC
...
!!!!!! =====================================
   ...
   ! find direct gap
    ...
    DO ik=1,kMax
       ...
       END IF
    END DO
    direct_gap_kpoint = 101 !Gamma point always
   ...
   write(34,*)"Ecc=",Ecc
   write(121,*)"Evv=",Evv
   if(casito.eq.1)write(121,*)"Ef=Evv+shift1"
   if(casito.eq.2)write(121,*)"Ef=Evv+shift2"
   if(casito.eq.3)write(121,*)"Ef=Evv+shift3"
   write(121,*)"Ecc=",Ecc
...
!    DO ik=1,kMax
    DO ik=1,230
      IF ((energy(ik,nVal+CCC)-energy(ik,nValwish)).gt.bajoNivel) THEN
       IF ((energy(ik,nVal+CCC)-energy(ik,nValwish)).LT.altoNivel) THEN
        counter=counter+1
      write(34,'(a9,1I6,a5,1I8,a2,1f8.3,a9,1I6,a2,1f8.3,a15,i4)')"set arrow",counter,"from",ik,",",(energy(ik,nValwish)),"-Evv to",ik,",",(energy(ik,nVal+CCC)),"-Evv nohead lt ",lt
      write(121,'(a9,1I6,a5,1I8,a2,1f8.3,a9,1I6,a2,1f8.3,a15,i4)')"set arrow",counter,"from",ik,",",(energy(ik,nValwish)),"-Evv to",ik,",",(energy(ik,nVal+CCC)),"-Ef nohead lt ",lt
  ...
  ! write(34,'(*)'set label "x=',Deltita,'at',nMax/2,'5'
   ...
   write(34,*)"plot '",trim(energy_data_filename),"' u 1:($",nValwish+1,"-Evv) title '1' w l lt ", lt, "lw 2,\"
   write(34,*)"     '",trim(energy_data_filename),"' u 1:($",(nVal+1+CCC),"-Evv) title '1' w l lt 1 lw 2"
   write(121,*)"Arows=",counter 
   write(121,'(a6,a29,a8,i3,a24,i3,a8)')"plot ",trim(coma)//trim(sigma)//trim(energy_data_filename)//trim(coma)," u 1:($",nValwish+1,"-Evv) title '1' w l lt ", lt, "lw 2,\"
   write(121,'(a29,a8,i3,a24,i3,a8)'),trim(coma)//trim(sigma)//trim(energy_data_filename)//trim(coma)," u 1:($",nVal+1+CCC,"-Ef) title '1' w l lt ", lt, "lw 2"



   
   call system('mv fort.34 salida.g')
   call system('mv fort.121 flechas')
...
deallocate (energy,difer)
 ...
 write(*,*)"salida es: flechas "
 write(69,96)CCC,nValwish,cual,deltita
96 format(2i4,f5.3,f5.2)
end program guagua
