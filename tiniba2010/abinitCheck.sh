...
## CHILDREN ::
## inversion.master, inversion.quad, inversion.itanium, inversion.node
## LAST MODIFICATION :: Martes 11 Marzo 2010 
## LAST MODIFICATION :: Martes 16 Marzo 2010 a las 10:50 by jl 
  RED='\e[0;31m'
 BLUE='\e[0;34m'
  BLU='\e[1;34m'
 CYAN='\e[0;36m'
...
### define in nice way.
 ...
 DIRCHECK=$CASO'_check'
   ...
   #### Great cabellos vaya hasta que la cuajaste.    
   ABINIS=/home/prog/abinit/abinitv4.6.5_"`hostname  | sed 's/[0-9]//g'`"/abinis."`hostname  | sed 's/[0-9]//g'`"
    INVER=$WHERE/SRC_inversion/inversion."`hostname  | sed 's/[0-9]//g'`"
      SUM=$WHERE/SRC_suma/suma."`hostname  | sed 's/[0-9]//g'`"
 ...
 if [ ! -e .machines_pmn ];then 
   printf  "\tThere isnt FILE : .machines_pmn ; make one ...\n"
 fi 
 if [ ! -e .machines_scf ];then 
   printf  "\tThere isnt FILE : .machines_scf ; make one ...\n"
 fi 
####################
####################
####################
  if [ "$1" == '1' ];then
    if [ -d  "$DIRCHECK" ];then 
      printf "\tDirectory ${BLUE}$DIRCHECK${NC} existe .\n"
      printf "\tDo you want to remove?         [ yes=borrar  anyother=no ] \n"
      read ANS
       if [ "$ANS" == "yes" ];then 
        printf "\tErasing dir ${BLUE}$DIRCHECK${NC}\n" 
        rm -rf "$DIRCHECK"
       else 
        printf "\tYou have to erase directory : ${BLUE}$DIRCHECK${NC} \n" 
...
dir=$PWD
ab_exec_xeon=$ABINIS  # /home/narzate/share_new/abinit-4.6.5/abinis
where=$WHERE               # $HOME/abinit_shells/utils
whereset=$WHERE            #$HOME/abinit_shells/clustering/itaxeo
...
# looks for centrosymmetry
    ...
    cp symmetries/sym.d fort.9
     ...
     if [ ! -e "$INVER" ];then 
       ...
       printf "\t$INVER  \n"
       printf "\tStop right now ...fix it pleaase and run again ...\n\n"
       exit 0
     else 
        printf "\t`dirname $INVER`/${GREEN}`basename $INVER`${NC}\n"
        sim=`$INVER`
     fi 
    rm fort.9
    if [ $sim == 'scs' ]
...
#    echo $a10 $a20 $a30 >> symmetries/pvectors
    ...
    #echo "perro"
     ...
     if [ ! -e $SUM ];then 
      printf "\tHold on: there isnt file $SUM\n"
     else 
      echo $z1 $z2 $a3 | $SUM
     fi
