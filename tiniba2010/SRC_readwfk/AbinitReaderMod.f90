...
102    FORMAT(I6,280F21.16)
    END IF
    
    IF (nbandwf.GT.280) THEN
       WRITE(6,*) 'WF READER IS SET TO ONLY WRITE OUT 80 BANDS MAXIMUM'
...
!       WRITE(51,*) nkpt
    ...
    IF (sme) THEN
       OPEN(UNIT=61,FILE='spinmn.d')
       OPEN(UNIT=62,FILE='entanglement.d')
    END IF
       ...
       DO jband = 1, nbandwf
          ...
          DO ii = 1, npw
             ...
             ELSE IF (nspinor==2) THEN
                ...
                entanglement = upup*upup + dndn*dndn + 2.d0*updn*dnup
...
105          FORMAT(6E16.8)
          ...
          IF (jband.EQ.iband) THEN
             WRITE(UNIT=62,FMT=105) entanglement
          END IF
