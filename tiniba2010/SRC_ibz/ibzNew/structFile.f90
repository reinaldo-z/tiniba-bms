...
!!! iz are the symmetries from the struct file
...
CONTAINS
  ...
  SUBROUTINE structReader
    ...
    READ(*,*) case
    IF (debug) WRITE(*,*) "Case is: ", TRIM(case)
    
    filename = TRIM(case)//'.struct'
    IF (debug) WRITE(*,*) "(File Control) Opening file: ", filename
    
    OPEN( UNIT=20, FILE=filename, STATUS="OLD",IOSTAT=istat)
    
    IF (istat.NE.0) THEN
       WRITE(*,*) "ERROR: Cannot open file "//TRIM(filename)
       STOP "Error opening struct file."
    END IF
...
545    FORMAT ( A10, 5X, I5, 5X, F10.9, 5X, F10.8  )
    ...
    IF (debug) WRITE(*,'(A44,I3)') "The number of symmetry matrices expected is ", iord
    ...
    nSym = iord
    ALLOCATE( symMatsConv(3,3,nSym) )
    symMatsConv(1:3,1:3,1:nSym) = 0
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  SUBROUTINE convertWienDataToInternalData()
    ...
    CALL SymmetryMatrices()
    
  END SUBROUTINE convertWienDataToInternalData
  
  
  SUBROUTINE SymmetryMatrices()
...
!!! Write out the matrices
    ...
    IF (printSymmetries) THEN
       OPEN(UNIT=1,FILE="Symmetries.Conventional")
       WRITE(1,*) "The symmetry matrices in the conventional systems"
