MODULE myAllocateMod
  ...
  INTERFACE myAllocate
     ...
     MODULE PROCEDURE myAllocateMatrix
  END INTERFACE
...
CONTAINS
  ...
  SUBROUTINE myAllocateMatrix (array, stringArrayName, arraySize)
    
    IMPLICIT NONE
    TYPE(matrix3by3), ALLOCATABLE :: array(:)
    TYPE(matrix3by3) :: zeroMatrix
    CHARACTER(LEN=*), INTENT(IN) :: stringArrayName
    INTEGER, INTENT(IN) :: arraySize
    INTEGER :: istatus
    ...
    ALLOCATE ( array(arraySize), STAT=istatus )
    IF (istatus .EQ. 0) THEN
       IF (debug) WRITE(*,*) "(myAllocateMatrix): "//TRIM(stringArrayName)//" allocated"
    ELSE
       WRITE(*,*) "Could not allocate "//TRIM(stringArrayName)//".  Stopping."
       STOP "Could not allocate array."
    END IF
    
    zeroMatrix%el(1:3,1:3) = 0
    array(:) = zeroMatrix
  END SUBROUTINE myAllocateMatrix
