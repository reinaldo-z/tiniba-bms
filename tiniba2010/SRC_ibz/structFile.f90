...
!!!    filename = "Si100c.struct"
    ...
    !!!! here i have modificated by jl cabellos
    !!!!! ==================================
     CALL getcwd(cwd) 
     call caso(cwd,cazo)
     filename = TRIM(cazo)//'.struct'
    inquire (file=trim(filename),exist=filexist)
          if (.NOT. filexist) then
             call system ("rm -f killme")
             WRITE(*,*)"              "
             WRITE(*,*)"============================"
             write(*,*)"       Error, missing file:",trim(filename)
             write(*,*)"       Stoping rigth now..jl"
             write(*,*)"       structFile.f90 L=13"
             WRITE(*,*)"============================"
             call system ("touch killme")
             stop
          end if

     !ii1cab=(index(cwd(1:180),'/',BACK = .TRUE.))
     !case=(trim(cwd(ii1cab+1:180)))
     !!!write(*,*)(trim(cwd(ii1+1:180))) !debug
