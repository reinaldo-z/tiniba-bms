#!/bin/bash
## Jose Luis Cabellos 
## report bugs <sollebac@gmail.com>
GREEN='\e[0;32m'
green='\e[1;32m'
YELLOW='\e[1;33m'
red='\e[0;31m'
...
###################################################
  for ((hh=0;hh<=($NOMACHINESpmn-1); hh++));do
    let "HHSU=$hh+1"
       ...
       LOCA=${LOCAL[$hh]}
        ...
        echo "rsh `hostname` \"touch $LOCA/abineta\"" >> abinis_$HHSU.sh 
        echo "rcp log `hostname`:$LOCA" >> abinis_$HHSU.sh 
 ...
 echo "\$ABIRUN < \$ABINITFILES > log " >> abinis_$HHSU.sh 
     ...
     echo " if [ \"\$SAL\" -lt \"4\" ];then " >> abinis_$HHSU.sh          
          ...
          echo "fi"   >> abinis_$HHSU.sh                                      
        echo "fi"   >> abinis_$HHSU.sh                        
       BASEli=`basename $LOCA`
       printf "\t${GREEN}Creating${NC}:  ${BLUE}$BASEli${NC}/abinis_$HHSU.sh \n"
